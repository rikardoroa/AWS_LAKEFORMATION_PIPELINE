...
#                 logger.warning(f"Table {self.table_name} not found in database {self.database_name}")
...
class DBUtils:
    """
    Utility class simplified - table creation is handled by Glue Crawler.
    """
    ...
    def verify_table_exists(self):
        """
        Simply verify if the table exists after crawler runs.
        """
        try:
            response = self.glue_client.get_table(
                DatabaseName=self.database_name,
