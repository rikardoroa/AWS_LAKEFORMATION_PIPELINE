...
data "aws_iam_policy_document" "pipeline_dev_policy_cb_api" {
  ...
  # 🔹 Permisos Kinesis (🔥 FIX APLICADO)
  # Mantenemos tu var.kinesis_stream_arn, pero añadimos wildcard como fallback
  statement {
    sid    = "KinesisAccess"
    effect = "Allow"
    actions = [
...
# 🔹 Permitir invocación desde EventBridge Scheduler
resource "aws_lambda_permission" "allow_scheduler_invoke" {
  statement_id  = "AllowEventBridgeInvokeLambda"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lambda_function.function_name
